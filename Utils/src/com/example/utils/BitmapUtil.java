 ...
 * <p/>
 * <b>图片存在的三种形式</b> 1、文件形式(即以二进制形式存在于硬盘上) 2、流的形式(即以二进制形式存在于内存中) 3、Bitmap形式
 ...
 * <p/>
 *
 * <b>图片压缩总结</b> 总结来看，图片有三种存在形式：硬盘上时是file，网络传输时是stream，
 ...
 * 并不会改变图片的像素，所以就算质量被压缩了，但是bitmap在内存的占有率还是没变小， 但你做成file时，它确实变小了；
...
public final class BitmapUtil {
	 ...
	 *            图片路径
	 * @param pixelW
	 *            target pixel of width 800f
	 * @param pixelH
	 *            target pixel of height 480f
	 * @return
	 */
	 ...
	 * @throws IOException
	 */
	public Bitmap compressImage(Bitmap bm, double maxSize) {
		// 图片允许最大空间 单位：KB
	...
	/**
	 * 获取Bitmap大小(在内存中占的大小是文件本身大小的4倍，所以计算实际大小时 /4) 检测图片三种形式大小的方法: 文件形式:
	 * file.length() 流的形式: 讲图片文件读到内存输入流中,看它的byte数 byte[] b = baos.toByteArray();
	...
	private static BitmapFactory.Options getBestOptions(
		...
		mDesiredWidth = getResizedDimension(reqWidth, reqHeight, actualWidth,
				actualHeight);
		mDesiredHeight = getResizedDimension(reqHeight, reqWidth, actualHeight,
				actualWidth);
		// 根据现在得到计算inSampleSize
		options.inSampleSize = calculateBestInSampleSize(actualWidth,
				actualHeight, mDesiredWidth, mDesiredHeight);
	 ...
	 */
	// Visible for testing.
	private static int calculateBestInSampleSize(int actualWidth,
			int actualHeight, int desiredWidth, int desiredHeight) {
	 ...
	 */
	private static Bitmap createScaleBitmap(Bitmap tempBitmap,
		...
		if (tempBitmap != null
			...
			Bitmap bitmap = Bitmap.createScaledBitmap(tempBitmap, desiredWidth,
					desiredHeight, true);
	 ...
	 *            从输入流中读取Bitmap
	 * @param outPadding
	 *            如果不为null，则返回填充bitmap，如果它存在，否则设置填充为[-1，-1，-1，-1]。
	 *            如果没有位图返回（空），那么填充不变。
	 * @param reqWidth
	 *            目标宽度
	 * @param reqHeight
	...
	/**
	 * 获取一个指定大小的bitmap来自输入流 实际调用的方法是bitmapFromByteArray(data, 0, data.length, w,
	 * h);
	 *
	 * @param is
	 *            从输入流中读取Bitmap
	...
	/**
	 * 返回图片的缩略图. 缩略图的大小定义的维度 这种方法不是线程安全的,应该只在UI线程上调用。
	 *
	 * @param bitmap
	 * @param context
